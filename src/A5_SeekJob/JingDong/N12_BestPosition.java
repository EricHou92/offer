...
public class N12_BestPosition {
    ...
    public static void main(String[] args) {
        ...
        while (input.hasNextInt()) {
            ...
            Point start = new Point((cs[0].x + cs[1].x + cs[2].x) / 3, (cs[0].y
                    + cs[1].y + cs[2].y) / 3);
            double change = 10;
            ...
            if (fitness(start) < 1e-6)
                System.out.println(String.format("%.6f", start.x) + " " + String.format("%.6f", start.y));
            else {
    ...
    static double fitness(Point p) {
        ...
        for (int i = 0; i < 3; i++)
            as[i] = ang(p, cs[i], rs[i]);
        return (as[0] - as[1]) * (as[0] - as[1]) + (as[0] - as[2])
                * (as[0] - as[2]) + (as[1] - as[2]) * (as[1] - as[2]);
    }
